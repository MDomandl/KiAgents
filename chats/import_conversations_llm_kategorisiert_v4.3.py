...
def verarbeite_chat(chat, kategorien, stichwort_mapping, cursor):
    ...
    if kategorien_gesamt:
        cursor.execute(
            ...
            "ON DUPLICATE KEY UPDATE letzte_aenderung=VALUES(letzte_aenderung), message_count=VALUES(message_count), zusammenfassung=VALUES(zusammenfassung), status='überschreiben'",
...
def main():
    ...
    cursor.close()
    conn.close()
    print("✅ LLM-Kategorisierung (V4.3) mit Zusammenfassung abgeschlossen.")
