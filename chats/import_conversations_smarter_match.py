...
# Synonym-Mapping: schlüsselwort → kategoriename
SYNONYME = {
    ...
    "motor": "elektronik",
    "sensor": "elektronik",
    "stromsensor": "elektronik",
    "3d-druck": "3d druck",
    "druck": "3d druck",
    "verkabelung": "elektronik",
    "slam": "staubsaugerroboter",
    "ollama": "ai sonstiges",
    "noco": "datenbank",
...
def schlage_kategorien_smart(titel, zusammenfassung, kategorien):
    text = (titel + " " + zusammenfassung).lower()
    zuordnung = set()

    for synonym, ziel_kategorie in SYNONYME.items():
        if synonym in text and ziel_kategorie in kategorien:
            zuordnung.add((kategorien[ziel_kategorie], 3))

    # Zusätzlich klassisches Regex-Wortgrenzen-Matching
    for name, kat_id in kategorien.items():
        muster = r"\b" + re.escape(name) + r"\b"
        if re.search(muster, text):
            zuordnung.add((kat_id, 3))

    return list(zuordnung)
...
def verarbeite_chat(chat, bekannte_chats, kategorien, stichwoerter, cursor):
    ...
    if any(stichwort in titel.lower() for stichwort in stichwoerter):
        ...
        zuordnungen = schlage_kategorien_smart(titel, zusammenfassung, kategorien)
...
def main():
    ...
    for i, chat in enumerate(daten):
        print(f"🔄 Verarbeite Chat {i+1}/{len(daten)}: {chat.get('title', '')[:60]}")
        verarbeite_chat(chat, bekannte_chats, kategorien, stichwoerter, cursor)

    conn.commit()
    cursor.close()
    conn.close()
    print("✅ Import mit intelligentem Mapping abgeschlossen.")
