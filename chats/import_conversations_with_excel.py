...
def lade_excel_chat_infos(pfad):
    ...
    for index, row in df.iterrows():
        if pd.notna(row[0]) and pd.notna(row[1]):
            mapping[str(row[0]).strip().lower()] = str(row[1]).strip().lower()
    return mapping
...
def generiere_zusammenfassung_ollama(text):
    prompt = (
        ...
        f"Keine Einleitung oder Floskeln, keine Meta-Kommentare. Nur Inhalt.:{text}"
    )
    try:
...
def main():
    ...
    cursor.close()
    conn.close()
    print("✅ Import mit Ollama + manuellem Kategorie-Mapping abgeschlossen.")
