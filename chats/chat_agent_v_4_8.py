...
def extrahiere_kategorien_und_relevanz(text, kategorien_liste):
    ...
    for block in blocks[1:]:
        ...
        katname = re.sub(r"[^\wäöüß\-]", "", katname)  # nur sinnvolle Zeichen behalten
...
def generiere_kategorievorschlag(text, kategorien_liste):
    ...
    prompt = (
        ...
        f"* Verkehr: 4/5"
        f"Und gib für jede Kategorie eine Relevanz an wie: 4/5"
        f"Beispiel: \n* Bewerbung: 5/5"
        f"Bitte keine Zeile mit N/A ausgeben"
    )
    try:
    ...
    if kategorien_gesamt:
        cursor.execute(
            ...
            ON DUPLICATE KEY UPDATE
                letzte_aenderung=VALUES(letzte_aenderung),
        ...
        result = cursor.fetchone()
        if result:  
            if not braucht_llm_kategorisierung(result["id"], cursor):
                print(f"⏭️ Chat '{chat.get('title', '')}' wurde bereits LLM-kategorisiert – übersprungen.")
            else:
                chat_db_id = result["id"]

            # Alte Kategorien entfernen
                cursor.execute("DELETE FROM chat_kategorien WHERE chat_id = %s", (chat_db_id,))

                # Neue Kategorien eintragen oder aktualisieren
                for kat, rel in kategorien_gesamt.items():
                    if kat in kategorien:
                        cursor.execute(
                            ...
                            VALUES (%s, %s, %s, %s)
                            ON DUPLICATE KEY UPDATE relevanz = VALUES(relevanz)
                            """,
                            (chat_db_id, kategorien[kat], rel, "llama3")
                        )
    else:
...
def braucht_llm_kategorisierung(chat_id, cursor):
    cursor.execute(
        "SELECT COUNT(*) FROM chat_kategorien WHERE chat_id = %s AND quelle IN ('llama3', 'gpt4')",
        (chat_id,)
    )
...
def main():
    ...
    cursor.close()
    conn.close()
    print("✅ LLM-Kategorisierung (V4.5) abgeschlossen.")
