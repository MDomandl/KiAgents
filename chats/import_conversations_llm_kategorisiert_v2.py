...
def lade_excel_chat_infos(pfad):
    ...
    return [str(row.iloc[0]).strip().lower() for _, row in df.iterrows() if pd.notna(row.iloc[0])]
...
def generiere_kategorievorschlag(text, kategorien_liste):
    ...
    prompt = (
        ...
        f"Inhalt:/n{text}/n/n"
        f"Wenn der Inhalt sich auf KI, ChatGPT, Ollama, Sprachmodelle oder AI-Funktionen bezieht, "
        f"wähle 'AI' oder 'AI sonstiges'. Nur die Kategorie als einzelnes Wort zurückgeben."
    )
    try:
    ...
    if bereinigt and bereinigt in kategorien:
        ...
        if result:
            ...
            cursor.execute(
                ...
                (chat_db_id, kategorien[bereinigt], 1)
            )
...
def main():
    ...
    cursor.close()
    conn.close()
    print("✅ LLM-Kategorisierung (V2) abgeschlossen.")
